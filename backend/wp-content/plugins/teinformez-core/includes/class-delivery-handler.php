...
class Delivery_Handler {
    ...
    private function build_digest_html($user, $by_category, $frequency) {
        ...
        $used_image_ids = [];
        $thumbnail_budget = 1; // thumbnails in category sections (hero image is separate, total max 2)
        ...
        $sponsored = $wpdb->get_row($wpdb->prepare(
            "SELECT * FROM {$ads_table} WHERE status = 'active' AND campaign_start <= %s AND campaign_end >= %s ORDER BY id DESC LIMIT 1",
            $today_sql,
            $today_sql
        ));

        if ($sponsored) {
            // Use sponsor's custom HTML and increment impression counter
            ...
            if ($cas_enabled && !empty($cas_url) && !empty($cas_key)) {
                ...
                // MA alias-resolves `slot`; 'infeed' → WEBSITE_INFEED (live "pe TeInformez" cross-promo ads;
                // NEWSLETTER_INLINE ads are all SKIPPED, so reuse the active INFEED pool).
...
<body style="margin:0;padding:0;background-color:#f0f1f3;font-family:Arial,Helvetica,sans-serif;">
    <div style="max-width:600px;margin:0 auto;">
        ...
        <!-- Partner promo block (day-of-week rotation, always included) -->
        ' . $promo_html . '
