...
- [~] 🟡 **GDPR** — **consent-gating LIVE 2026-07-07**: GA4 NU se mai încarcă fără consimțământ explicit (gate pe `ti_cookies_consent`); banner cookie NOU pentru vizitatori anonimi (gap real: TIConsentGate arăta bannere doar userilor logați); accept-ul COOKIES din fluxul Legal deschide și el gate-ul. **Rămas**: CSP (Report-Only întâi) — sesiune dedicată (curatorierea origins: GA/Sentry/imagini).
...
- [ ] 🟡 **Juridic invizibil (Item 4, amânat la cererea user 2026-07-07)** — modul complet în backend, 0 rută publică Next.js. **Decizie produs**: expui public (rută `/juridic` + link footer/meniu) sau marchezi amânat oficial? (dublează item-ul „Juridic invizibil + H1" de mai sus — H1 e deja făcut, rămâne doar decizia de expunere).
- [ ] 🟢 **FOLLOW-ON backend: reader „primește știri pe Telegram"** — connect-flow real (reuse `@aledan/telegram`: mint nonce + webhook bind `telegram_chat_id` + livrare digest pe Telegram pt userii linkați), ca la eat/Tutor/4pro-client. Necesită: endpoint-uri WP noi + bot nou (BotFather, acțiune user) + coloană user + wiring în delivery-handler. Strategia deja cere asta (Telegram = canal reader). Estimat 1 sesiune dedicată. Până atunci: tool-ul admin e ascuns din contul reader (fix de mai sus).
- [ ] 🟢 **FOLLOW-ON backend: `/subscription/status` auth mismatch** — endpoint-ul folosește `require_auth = is_user_logged_in()` (doar sesiune-cookie WP), NU token-ul Bearer al frontend-ului → 401 pentru orice user logat prin frontend → statusul Premium nu se încarcă niciodată. Fix: rezolvă Bearer→user + `wp_set_current_user` (ca `is_authenticated` din `class-rest-api.php`) în loc de `require_auth`. Legat de Premium (out-of-scope în STRATEGY, dar LIVE — vezi conformanță). Frontend-ul acum degradează grațios la „Plan Gratuit" (fix Item 2), deci non-blocant pt free readers.
- [ ] 🟢 **Sincronizare STRATEGY.md** — Premium „out-of-scope" dar LIVE (ridică strategia, nu tăia) · hosting „Vercel"→VPS2 · Referral out→in nesincronizat.
