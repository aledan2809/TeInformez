...
- [ ] **G-TI-PHP-ANALYTICS-FOREACH-NULL** — NEW gap discovered 2026-05-15 while verifying Phase A (G-TI-PHP-NEWS-API-DEFENSIVE-READS). Nginx error log shows recurring `PHP Warning: foreach() argument must be of type array|object, null given in /var/www/teinformez-repo/backend/wp-content/plugins/teinformez-core/admin/views/analytics.php on line 250`. Recurs ~1× per minute (cron-fired admin page render). Different file + scope vs news-api defensive reads (unrelated codepath). Fix: read `admin/views/analytics.php:250` + guard the foreach with `(array)($var ?? [])` or `is_array($var) && foreach`. ~15min surgical. Logged here per `feedback_scope_discipline`; closes a recurring noise pattern in production logs.
