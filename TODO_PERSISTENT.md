...
- [x] **M5 — Share-to-unlock / referral — DONE 2026-07-13 (`277b2f9`+`a4f4205`, LIVE + verificat vizual)**: link personal de invitație → prietenul se înscrie prin `?ref=cod` → **ambii** primesc +7 zile Premium gratuit (dublu-față, plafon 60z). Payoff real = Premium (nu leaderboard). Premium-cadou = mecanism separat de Stripe (usermeta `granted_until` onorat de `/subscription/status`, money-path neatins). Anti-abuz variant A (guards: cod valid/self/test/o-recompensă-per-prieten; înscrierea n-are confirmare email → dacă abuz, adăugăm variant B). Pagina `/dashboard/invite` + nav. E2E prin endpoint real 201→ambii granted; vizual live OK. Detalii: ledger.
- [ ] **M6 — Distribuție** (~2 sesiuni, recon 2026-07-13 corectat — vezi L10 lessons-learned): clipuri scurte verticale (ELI12 + TTS, TikTok/Reels/Shorts) + auto-post **card ELI12** pe Telegram/FB (nu link) + SEO conținut ELI12 unic. **Corecție importantă**: fundația ELI12 EXISTĂ deja, LIVE din 2026-06-04 (backend `simple_explanation`+`why_it_matters` generate AI, `class-ai-processor.php`) și consumată pe frontend din 2026-07-07/10 (secțiunea „Pe scurt" pe `/news/[id]`, `NewsDetailClient.tsx`) — un recon superficial anterior declarase greșit că lipsește. Ce chiar lipsește pentru M6: (a) TTS/citire audio a textului ELI12 (vezi și OPT-1 mai jos, se suprapun — posibil unificate într-o singură felie); (b) pipeline video (montaj+voce, cross-project cu generatorul de reels planificat în `MarketingAutomation/TODO_PERSISTENT.md`, extern TikTok/Reels API); (c) card-post (nu link) cu conținut ELI12 pe Telegram/FB (`Social_Poster` postează azi cu link, nu card ELI12; Telegram n-are broadcast pe canal, doar DM la cititori conectați — infra nouă); (d) opțional expunere SEO dedicată a ELI12 dacă „Pe scurt" de pe pagina articolului nu e suficient pentru indexare unică. Recomandare pentru sesiunea viitoare: pornește cu (c) card-post FB (mic, reuse `Social_Poster` existent) sau (a) TTS (unifică cu OPT-1), lasă (b) video pt sesiune dedicată separată (mare + extern).
- [~] **M6b — Instagram publishing + CAS pe social (analiză 2026-07-10, decizie: distribuție DA / bani direcți NU)** — **IG publishing CORE LIVE (inert) 2026-07-12 (`325d7a1`)**: `Social_Poster` extins cu Graph API 2-pași (`media`→`media_publish`), caption titlu+ELI12+UTM (`utm_source=instagram&utm_medium=social`), log `instagram_post` (user_id=NULL), `retry_failed_posts` extins; skip fără imagine (IG cere imagine); inert până la `teinformez_instagram_business_id`. **RĂMAS**: (user) cont IG Business legat de pagina FB + token cu `instagram_content_publish`+`instagram_basic` (tokenul MA actual are doar `pages_manage_posts`) → apoi `wp option update teinformez_instagram_business_id <id>` + test E2E real; (follow-on) **CAS-pe-social** (max 1/N postări = creative MA promo, UTM `utm_medium=cas`) → **BLOCAT pe MA 2026-07-12**: cere endpoint structurat nou `GET /api/cas/social` (JSON `image_url`+`caption`+`link`), nu HTML-ul actual `/api/cas/render`; build-prompt detaliat scris în `MarketingAutomation/TODO_PERSISTENT.md`. Consumatorul TeInformez (contor rotație + fetch + post FB/IG) = mic, după ce endpoint-ul MA există. Concluzii analizate cu cod + prod verificate:
...
- [x] **CAS-06 — Launch Plan attribution emitter — DONE 2026-07-13 (`7202fb1`)**. Finding: `MA_Emitter` era **deja construit+activ** (cron, emite USER_REGISTERED/newsletter/article către MA cu `X-Webhook-Secret`); gap-ul real = `utm_content` (trackingCode) se pierdea pe lanțul de signup → signup-uri `SKIPPED` (neatribuite). Fix aditiv 6 fișiere: utm.ts captează utm_content + register forward + auth-api usermeta `teinformez_utm_content` + emitter LEFT JOIN. E2E verificat pe prod (register 201 → usermeta → JOIN întoarce trackingCode; test user șters). Detalii: ledger. **Analog rămas (opțional)**: `NEWSLETTER_SUBSCRIBED` la fel nu poartă utm_content → fix similar dacă se vor atribui abonările la newsletter din CAS. Original pending description mai jos pt istoric:
  <!-- original -->
- [~] **CAS-06 — original description (superseded de DONE de mai sus)** — adăugat 2026-05-22 din MA Launch Plan Final Verification. CAS-04/05 acoperă render-side (TeInformez **trage** reclame din MA). Direcția inversă NU e încă wired: TeInformez ar trebui să **emită** evenimente de attribution (`TEINFORMEZ_USER_REGISTERED` etc.) către MA `POST https://ma.techbiz.ae/api/external/teinformez/events` cu `X-Internal-Key` + `trackingCode` (din redirect-ul CAS click `utm_content`), ca MA să atribuie signups/conversii la `LaunchPlanAction.metrics`. **MA-side receiver e gata** (`/api/external/teinformez/events`, atomic JSONB increment — vezi `MarketingAutomation/knowledge/launch-plan-module.md`). **Emitter TeInformez deferred** — sesiune dedicată propose-confirm-apply (RESTRICT). Estimat ~1-2h: hook pe user-register WP action → POST către MA cu trackingCode capturat la signup.
...
- [~] 🟢 **Telegram v2 — cadență telegram-only + canal în UI — COD LIVE 2026-07-10** (`5ac61b8`+`0910d18`+`5d57e22`; walk Playwright live 9/9 PASS; bonus: fix bug preexistent upsert `update_preferences` — salvarea Setări era no-op silențios pentru useri fără rând de preferințe): delivery-handler acceptă useri telegram-only (gate: bot configurat + chat linkat; fail-closed → identic v1 până la activarea botului); dedupe `channel IN (email, telegram)` (semantică: digest ajuns pe ≥1 canal = done pe fereastră); email condiționat pe canal; **bind-ul din bot bifează automat canalul telegram** (idempotent); ChannelSelector: card Telegram adăugat + Facebook/Twitter retrase la „În curând" (nu livrau nimic pentru readeri; rămân debifabile pentru cine le avea) + sumar doar pe canale care livrează + hint „Telegram devine activ după conectare". **DIGEST PROGRAMAT VERIFICAT E2E + BUG FIX 2026-07-12** (`2ed5586`): sistemul de digest programat pe Telegram (cron `teinformez_check_deliveries`@15min via cron sistem fiabil, `DISABLE_WP_CRON=1`) verificat pe output real (reader 9 provizionat izolat via reflection pe `send_digest`, zero atingere useri reali): scheduler-ul selectează corect user 9 (`DUE_DAILY_NOW:[9]`), `send_digest`→`send_message`=**true** (Telegram acceptă mesajul). **Bug prins prin execuție (L05)**: `delivery_log.channel` era ENUM fără `'telegram'` → livrările telegram se logau cu channel `''` → **dedupe-ul v2 `channel IN (email,telegram)` NU le vedea** (telegram-only re-procesat inutil + invizibil în analytics). Fix: adăugat `'telegram'` la enum (activator + ALTER prod additiv). Validat post-fix pe prod: write channel='telegram' stochează corect (era ''). Date test curățate (L07).
...
- [x] 🔵 **next@16 upgrade — DONE 2026-07-13 (`580b5ed`+`96faf70`, LIVE + verificat browser real)**. Livrat combinat Next 14→16.2.10 + React 18→19.2.7 (build `--webpack` = același bundler ca 14; `.npmrc legacy-peer-deps` + `@stripe/*` explicit pt stripe-module vendored; `outputFileTracingRoot:__dirname` pt standalone non-nested; 1 fix type `useRef`). Verificat LIVE: SSR 200 + hidratare React 19 (10 articole client-fetched, 12 iconițe lucide, 17 butoane) + 0 erori console + re-walk CSP 0 violări. Detalii: ledger `reports/DIRECT-CHANGES-2026-07.md`. **Follow-on non-blocker**: eslint-config-next→16 (eslint 9 flat-config), trecere webpack→Turbopack (Sentry auto-detect + `sentry.*.config.ts`→`instrumentation-client.ts`), opțional framer-motion→12/zustand→5.
  <!-- assessment original păstrat mai jos pt istoric -->
- [x] 🔵 ~~next@16 upgrade — assessment~~ (superseded de DONE de mai sus). Frontend e pe **Next 14.2.35 + React 18 + Tailwind 3** (CLAUDE.md proiect zice „16+React19" = greșit). Target 16 = **2 majore (14→15→16) + React 18→19**. **Suprafață migrare (verificată)**: ✅ ZERO `cookies()`/`headers()`/`draftMode()` în cod (app-ul folosește js-cookie client + Bearer) — lipsește migrarea async-dynamic-API cea mai grea; ⚠️ **~5 fișiere** cu `params`/`searchParams` devin `Promise` în 15, cer `await` (`app/news/[id]/page.tsx` + route handlers `api/consent/document`, `api/auth`, `api/consent/status`, `api/analytics`); 🔴 **React 18→19** = riscul principal — bump deps compat: `lucide-react ^0.321` (vechi), `framer-motion ^11.18`, `react-hook-form ^7.50`, + verificat `@sentry/nextjs ^10.52` pe Next16/React19; 🔴 **caching semantics Next 15** (`fetch` nu mai e cache-uit default → audit `revalidate`/freshness pe fetch-urile WP API); Turbopack default în 16. **Plan (dedicat)**: (1) 14→15 (bump next+eslint-config-next, `await params/searchParams` 5 fișiere, audit caching, build VPS+E2E); (2) React 18→19 (bump react/react-dom/@types + lucide/framer-motion/react-hook-form la versiuni React19, tsc); (3) 15→16 (bump final, Turbopack, Sentry, build VPS); (4) TWG-GW + journey walk + **re-walk CSP** (headerele enforce din 2026-07-13) + deploy gated pe exit code. RESTRICT → propose-confirm + build PE VPS (nu local, memory `no-prod-build-while-dev-running`).
  > **Progres 2026-07-13 (cont.) — de-risc parțial DONE (`2f5cbbe`)**: **CORECȚIE plan** — Next 15 App Router are React 19 **cerință dură** (nu peer-recomandare) → pașii (1)+(2) **NU se pot separa**, se contopesc (bump la 15 trage React 19 obligatoriu; React 18 rămâne doar pe Pages Router). **Recon: suprafața de cod 14→15 = ~zero** (deja forward-compat: `news/[id]` are `params:Promise`+`await`; toate fetch-urile server au `next:{revalidate:N}` explicit; `next.config.js` curat; `@sentry/nextjs` v10 OK). **Audit dep-uri React 19**: framer-motion 11.18.2, zustand 4.5.7, next-themes 0.4.6, react-hook-form (instalat 7.71.1) = **deja React-19-ready**. Singurul vechi = **lucide-react 0.321** (fără `^19`) → **bump-uit la 0.469.0** (ultima cu React-19-peer ȘI toate 58 iconițe, brand icons scoase la 0.475) + rhf floor→7.71.1. Zero UI change, build VPS exit 0, deploy live 200. **Rămas pt sesiunea 15/19**: bump `next`+`react`+`react-dom`+`@types/react` la 19, apoi 15→16 (Turbopack, Sentry), TWG-GW + journey walk + **re-walk CSP** (headere enforce). Suprafața reală = doar bump-urile Next/React + verificare runtime React 19 breaking-changes (app nu folosește `ReactDOM.render`/PropTypes/string-refs → risc mic).
- [x] 🟢 **Dedup `news_archive` + prevenție recurență — DONE 2026-07-13** (`ab2970d`): archive avea 47.693 rânduri / 27.166 URL-uri distincte = **20.527 redundante (43%)**, până la 5 copii/URL. Root cause: `cleanup_old_items(30)` (cron zilnic `teinformez_daily_cleanup`) re-arhiva URL-uri re-fetch-uite cu `INSERT...SELECT` fără guard. Fix: dedup one-time pe prod (backup 266MB → DELETE 20.527, păstrat MIN(id)/URL → 27.166 rânduri, 0 redundante) + `UNIQUE KEY uq_archive_url(original_url)` (drop prefix redundant) + cod `INSERT IGNORE` în `cleanup_old_items` (schema-of-record în activator). **0 bookmarks afectate** (site-ul are 0 bookmarks); feed default queue-only (`archive:false`) → zero schimbare vizibilă. Backup: `VPS2:/root/backups/teinformez-news_archive-pre-dedup-2026-07-13.sql`.
