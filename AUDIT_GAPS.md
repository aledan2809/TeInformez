...
| G-TI-CAS-XSS-RISK | MEDIUM | CAS-04 (commit `0886392`) renders MA ad creative via `dangerouslySetInnerHTML` in `frontend/src/components/home/InFeedAd.tsx:60` AND newsletter `build_digest_html` injects MA HTML body raw into `$promo_html`. Theoretical with zero inventory + MA first-party; becomes real XSS vector when ad creative is uploaded by less-trusted parties (self-serve advertisers) or if MA is compromised. | Eliminated — `b533883` (2026-05-15): centralized DOMPurify sanitization in `useCasSlot.ts` hook with `FORBID_TAGS: ['iframe','object','embed']` (script + inline handlers stripped by default) — protects both InFeedAd + BannerSlot consumers. Backend: new private `sanitize_promo_html()` helper in `class-delivery-handler.php` with explicit wp_kses allowlist (a/img/table/p/h1-6/styled containers), applied at both `$promo_html` assignment sites (sponsor banner L614 + MA CAS fallback L637). Smoke: DOMPurify config 7/7 (script/iframe/object/embed/svg-onload/event-handler all stripped; normal ad markup preserved). Live verify post-deploy: teinformez.eu / + /news 200; backend sanitize_promo_html present 3x in deployed file; frontend FORBID_TAGS + DOMPurify bundled in news + banner chunks. |
...
| G-TI-W3-003 | LOW | `/juridic` nav link returns HTTP 404 on live site (teinformez.eu/juridic). Route exists in `.journey-audit.json` nav config but page likely unpublished or removed. Stale nav entry. | Eliminated — `852a989` 2026-05-17: removed from .journey-audit.json nav config |
